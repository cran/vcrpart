...
Forthcoming in \textit{Journal of Statistical Software}
...
\begin{Schunk}
\begin{Sinput}
R> UCBA <- as.data.frame(UCBAdmissions)
...
\includegraphics{Figures/jss2564-ucba-fig-vcmL}
...
\includegraphics{Figures/jss2564-ucba-fig-cv}
...
$\overline{\text{err}}^{\text{cv}}_{28} = 1.148$
at $\hat{\lambda}=5.1$. The
original fit for~\code{vcm.UCBA} can be pruned by
$\hat{\lambda} = 5.1$ with the
command:
...
\includegraphics{Figures/jss2564-ucba-fig-vcm}
...
\includegraphics{Figures/jss2564-details-mean-centering}
...
\begin{Schunk}
\begin{Sinput}
R> control <- tvcglm_control(folds = folds_control(seed = 13))
\end{Sinput}
\end{Schunk}
%TC:endignore
...
commands, include $724$ observations on the seven
...
\includegraphics{Figures/jss2564-pima-fig-tree}
...
% Mon Sep  5 23:49:03 2016
\begin{table}[tbp]
\centering
\begingroup\small
\begin{tabular}{lrrr|rr|rr}
  \hline & \multicolumn{3}{c}{Misclassification} & \multicolumn{2}{c}{Complexity} & \multicolumn{2}{c}{Time} \\ & Boot & Boot-Mean & Orig & Boot & Orig & Boot & Orig \\ 
  \hline
...
\end{tabular}
\endgroup
\caption[Performances for the \code{Pima} data]{Performances for the \code{Pima} data. Boot: Medians (and means under Boot-Mean) of results from fits on 250 bootstrap samples. Orig: Results on the original data. Misclassification: Misclassification errors. Complexity: The number of coefficients plus the number of splits. Time: Computation time in seconds.} 
...
\includegraphics{Figures/jss2564-pima-fig-comp}
...
\begin{figure}[bt]
  \centering
  \setkeys{Gin}{width=1\textwidth}
\includegraphics{Figures/jss2564-pima-fig-simulations}
...
\begin{Schunk}
\begin{Sinput}
R> vcm.School <- tvcglm(formula = f.School, data = Schooling,
...
\includegraphics{Figures/jss2564-school-fig-cv}
...
\includegraphics{Figures/jss2564-school-fig-tree}
...
consists of $9$ terminal nodes. The tree
...
$6,180$ women who gave birth in June
...
$-0.23$ (corresponding to an odds ratio
of $e^{-0.23} =
0.79$) and is significant
...
\begin{Schunk}
\begin{Sinput}
R> vcm.PL <- tvcglm(formula = f.PL, family = binomial(),
...
\includegraphics{Figures/jss2564-pl-fig-cv}
...
13.49$. The error curve is relatively
...
\includegraphics{Figures/jss2564-pl-fig-tree}
...
-0.23$. The moderation effects of
...
increases $\hat{\beta}^{\texttt{july}}$ by 0.22,
and living in Vienna (W) or Lower Austria (NOe) increases
$\hat{\beta}^{\texttt{july}}$ by 0.27. These
